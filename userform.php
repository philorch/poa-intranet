...
<table border="0" cellspacing="0" cellpadding="0" width="90%" style="margin-left:100px;">
   ...
   print("</select></td> </tr>             
      ...
      <table><tr><td>&nbsp;</td></tr>  <tr>
        <td width='100'> <font face='Arial, Helvetica, sans-serif' size='2'> User Location</font> </td> 
		
	<td  width='120' ><font face='Arial, Helvetica, sans-serif' size='2'> New Hardware </font></td>
        <td  width= '120'><font face='Arial, Helvetica, sans-serif' size='2'> New Phone </font></td>
        <td  width= '120'><font face='Arial, Helvetica, sans-serif' size='2'> New Position</font></td>
   ...
   print("<p style='font-family:Arial, Helvetica, sans-serif; font-size:14px;'><strong>Please check off the email groups that the new user should be added to:</strong></p>");
   print("<table>\n<tr>\n");
   ...
   if ( $info["count"] != 0 ) {
	...
	print("<br/><br/>
		<table >
		<td valign='baseline' height='1'> <p style='font-family:Arial, Helvetica, sans-serif; font-size:14px;'> <b>File Rights:</b> Please list the folder names that user will need access to. If needs entire department, list departments.</p> </td> </tr>
 		<tr> <table ><tr>
		...
		$rowCnt =5;
		print("<p style='font-family:Arial, Helvetica, sans-serif; font-size:14px;'> <br/><br/><b>Please check off all Department applications needed </b></p>");
		print("<table > ");
		while ( $Row1 = mysql_fetch_array($Result) ) {
			if ( $rowCnt % 5 == 0 ) {
				if ( $rowCnt <> 5 ) {
	...
	if ( $err_status == 0 ) { // If no error in filling up data, then insert data into the database.
		if ($form_type == 'NEW') {
			...
			if ( mysql_db_query ('app',$InsertStmt,$Link) ) {
				...
				print("New user application for $_POST[first_name] $_POST[last_name] has been uploaded and notice email has been sent to HR for approval. ");  print ("<br> <a href= \"human-resources\">Back to HR page </a>");
